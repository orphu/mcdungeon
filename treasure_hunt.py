...
class TreasureHunt (object):
    ...
    def __init__(self,
        ...
        self.args = args
        self.dinfo = {}
        self.dinfo['thunt_name'] = cfg.thunt_name
    ...
    def generate(self, cache_path, version):
        ...
        if (located is True):
            ...
            self.tile_ents[Vec(0, 0, 0)] = encodeTHuntInfo(self,
                                                             version)
            # Add to the thunt cache.
            key = key = '%s,%s' % (
                self.position.x,
                self.position.z,
            )
            self.thunt_cache[key] = 1
...
#                self.outputterminal()
    ...
    def findlocation(self):
        ...
        for p, d in sorted_p:
            d_chunks = set()
            d_chunks.add((p[0], p[1]))
            if d_chunks.issubset(all_chunks):
                if self.args.debug:
                    print 'Found: ', p
                self.position = Vec(p[0],0,p[1])
                if self.args.debug:
    ...
    def bury(self, manual=False):
        if self.args.debug:
            print 'Locating start point vertically...'
